...
namespace BDO_Localisation_AddOn
{
    static partial class APCorrectionInvoice
    {
        ...
        public static void formDataLoad(SAPbouiCOM.Form oForm, out string errorText)
        {
            ...
            {
                if (oForm.Mode == SAPbouiCOM.BoFormMode.fm_ADD_MODE && oForm.DataSources.DBDataSources.Item("OCPI").GetValue("U_BDO_WBID", 0).Trim() == "") //
                {
                }
                else
                {
        ...
        public static void setVisibleFormItems(SAPbouiCOM.Form oForm, out string errorText)
        {
            ...
            try
            {
                string docEntrySTR = oForm.DataSources.DBDataSources.Item("OPCI").GetValue("DocEntry", 0);
            ...
            else
            {
                if (pVal.EventType != SAPbouiCOM.BoEventTypes.et_FORM_UNLOAD)
                {
                    ...
                    if (pVal.EventType == SAPbouiCOM.BoEventTypes.et_COMBO_SELECT & pVal.ItemUID == "WBOper" & pVal.BeforeAction == false)
                    {
                        ...
                        oForm.Freeze(true);
                        BDO_WBReceivedDocs.comboSelect(oForm, Program.oIncWaybDocFormCrMemo, pVal.ItemUID, "Invoice", out errorText);
                        oForm.Freeze(false);
                    ...
                    if (checker && !pVal.BeforeAction && pVal.ActionSuccess &&  oForm.Items.Item("BDO_CNTp").Specific.Value.ToString()=="1")
                    {
                        
                        //int docEntry = Convert.ToInt32(oForm.DataSources.DBDataSources.Item(0).GetValue("CardCode", 0));
                        int docNum = 27;
                        ...
                        string updatequery = @"UPDATE ""OCPI""
                                                SET ""U_BDO_WBID"" = '""', ""U_BDO_WBNo"" = '""' WHERE ""DocNum"" = '" + docNum + "'";
        ...
        public static void uiApp_FormDataEvent(  ref SAPbouiCOM.BusinessObjectInfo BusinessObjectInfo, out bool BubbleEvent)
        {
            ...
            if (BusinessObjectInfo.EventType == SAPbouiCOM.BoEventTypes.et_FORM_DATA_UPDATE)
            {
                if (BusinessObjectInfo.BeforeAction)
                {
                    formDataAddUpdate(oForm, out string errorText);
                ...
                {
                    setVisibleFormItems(oForm, out string errorText);
                }
            }
            ...
            if (BusinessObjectInfo.EventType == SAPbouiCOM.BoEventTypes.et_FORM_DATA_ADD & BusinessObjectInfo.BeforeAction == false & BusinessObjectInfo.ActionSuccess == true)
            {
                ...
                {
                    cancellation(oForm, Program.canceledDocEntry, out string errorText);
        ...
        public static void cancellation(SAPbouiCOM.Form oForm, int docEntry, out string errorText)
        {
            ...
            try
            {
                JournalEntry.cancellation(oForm, docEntry, "163", out errorText);
            }
            catch (Exception ex)
            {
                int errCode;
