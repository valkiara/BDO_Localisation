...
namespace BDO_Localisation_AddOn
{
    static partial class BDOSAutomaticTasks
    {
        public static void importCurrencyRate()
        {
            ...
            try
            {
                ...
                if (startDate != endDate)
                {
                    ...
                    while (startDate <= endDate)
                    {
                        ...
                        CurrencyB1.importCurrencyRate( startDateStr, out errorText);
                        if (string.IsNullOrEmpty(errorText) == false)
                        {
                            info = BDOSResources.getTranslate("CurrenciesHaveBeenImportedUnSuccessfully") + " : " + startDateStr;
                            Program.uiApp.SetStatusBarMessage(info + "! " + errorText, SAPbouiCOM.BoMessageTime.bmt_Short, false);
                        }
                        startDate = startDate.AddDays(1);
                    }
            ...
            {
                info = BDOSResources.getTranslate("CurrenciesHaveBeenImportedUnSuccessfully");
