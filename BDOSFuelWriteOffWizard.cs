        ...
        public static void createForm(out string errorText)
        {
            ...
            if (formExist == true)
            {
                if (newForm)
                {
                    ...
                    string objectTypeConsT = "UDO_F_BDOSFUTP_D"; 
                    ...
                    for (int count = 0; count < oDataTable.Columns.Count; count++)
                    {
                        ...
                        else if (columnName == "FuelCDoc")
                        {
                            ...
                            oColumn.DataBind.Bind("FuelMTR", columnName);
                            oLink = oColumn.ExtendedObject;
                            oLink.LinkedObjectType = "UDO_F_BDOSFUCN_D";
                        }
            ...
            try
            {
                ...
                queryBuilder.Append(
                    ...
                    FROM 
                        ""@BDOSFUC1""
                    INNER JOIN 
                        ""@BDOSFUCN"" ON ""@BDOSFUCN"".""DocEntry"" = ""@BDOSFUC1"".""DocEntry"" 
                    INNER JOIN 
                        ""OITM"" ON ""@BDOSFUC1"".""U_ItemCode"" = ""OITM"".""ItemCode""
                    WHERE 
                        ""@BDOSFUCN"".""U_DocDate""<='");
                ...
                queryBuilder.Append("' ");
                queryBuilder.Append(@" AND ""@BDOSFUCN"".""U_DocDate"" >= '");
                queryBuilder.Append(from.ToString("yyyyMMdd"));
                queryBuilder.Append("' ");

                if (!cons)
                {
                    queryBuilder.Append(@" AND ""@BDOSFUC1"".""U_FuTpCode"" = '" + oConsType + @"'");
                }

                if (!veh)
                {
                    queryBuilder.Append(@" AND ""@BDOSFUC1"".""U_ItemCode"" = '" + oVehicle + @"'");
                }
                ...
                if (!type)
                {
                    queryBuilder.Append(@" AND ""@BDOSFUC1"".""U_FuelCode"" = '" + oFuelType + @"'");
                }
                ...
                while (!oRecordSet.EoF)
                {
                    ...
                    oDataTable.SetValue("AssetCode", rowIndex, oRecordSet.Fields.Item("U_ItemCode").Value);
                    oDataTable.SetValue("AssetName", rowIndex, oRecordSet.Fields.Item("U_ItemName").Value);
                    oDataTable.SetValue("ConsumType", rowIndex, oRecordSet.Fields.Item("U_FuTpCode").Value);
                    oDataTable.SetValue("StartUnit", rowIndex, oRecordSet.Fields.Item("U_OdmtrStart").Value);
                    oDataTable.SetValue("EndUnit", rowIndex, oRecordSet.Fields.Item("U_OdmtrEnd").Value);
                    oDataTable.SetValue("WorkHours", rowIndex, oRecordSet.Fields.Item("U_HrsWorked").Value);
                    oDataTable.SetValue("FuelType", rowIndex, oRecordSet.Fields.Item("U_FuelCode").Value);
                    //oDataTable.SetValue("FuelName", rowIndex, oRecordSet.Fields.Item("FuelName"));
                    oDataTable.SetValue("UomEntry", rowIndex, oRecordSet.Fields.Item("U_FuUomEntry").Value);
                    oDataTable.SetValue("NormConsum", rowIndex, oRecordSet.Fields.Item("U_NormCn").Value);
                    oDataTable.SetValue("Consum", rowIndex, oRecordSet.Fields.Item("U_ActuallyCn").Value);
                    ...
                    oDataTable.SetValue("Dimension1", rowIndex, oRecordSet.Fields.Item("U_Dimension1").Value);
                    oDataTable.SetValue("Dimension2", rowIndex, oRecordSet.Fields.Item("U_Dimension2").Value);
                    oDataTable.SetValue("Dimension3", rowIndex, oRecordSet.Fields.Item("U_Dimension3").Value);
                    oDataTable.SetValue("Dimension4", rowIndex, oRecordSet.Fields.Item("U_Dimension4").Value);
                    oDataTable.SetValue("Dimension5", rowIndex, oRecordSet.Fields.Item("U_Dimension5").Value);
                    oDataTable.SetValue("FuelCDoc", rowIndex, oRecordSet.Fields.Item("DocEntry").Value);
                    rowIndex++;
                    oRecordSet.MoveNext();
                }
            }
                ...
                if (sCFL_ID == "ItemMTR_CFL")
                {
                    ...
                    try
                    {
                        string queryVis = @"SELECT ""ItemCode""
                                                    From ""OITM""
                                                    INNER JOIN ""OACS""
                                                    ON ""OITM"".""AssetClass"" = ""OACS"".""Code"" AND ""OACS"".""U_BDOSVhcle"" = 'Y'";
